...
import { Palette, Clock, CheckCircle, Ban, X } from 'lucide-react'
...
import { managementDesigners } from '../data/databank'
...
const DesignerManagement = () => {
  ...
  const [selectedDesigner, setSelectedDesigner] = useState<DesignerRow | null>(null)
  const [showDrawer, setShowDrawer] = useState(false)
  const [showRejectModal, setShowRejectModal] = useState(false)
  const [isProcessing, setIsProcessing] = useState(false)

  const data = useMockData ? managementDesigners : []
  ...
  const metrics = [
    { label: 'Total Designers', value: data.length, icon: Palette },
    { label: 'Pending', value: data.filter((r) => r.status === 'pending').length, icon: Clock },
    { label: 'Approved', value: data.filter((r) => r.status === 'approved').length, icon: CheckCircle },
    { label: 'Suspended', value: data.filter((r) => r.status === 'suspended').length, icon: Ban },
  ]
  ...
  const columns: ColumnDef<DesignerRow>[] = [
    {
      key: 'name',
      header: 'Designer',
      render: (row) => (
    ...
    },
    {
      key: 'wallet',
      header: 'Wallet',
      render: (row) => <WalletCopy address={row.wallet} />,
    },
    {
      key: 'status',
      header: 'Status',
    ...
    { key: 'submittedCount', header: 'Submitted' },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Review designers and their submitted works"
      />
      ...
      <div className="sticky top-0 z-10 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="flex-1">
            <input
              type="search"
              placeholder="Search by designer name, wallet, or ID"
              value={searchQuery}
          ...
          >
            ...
            {DESIGNER_FILTERS.status.map((option) => (
          ...
          <select
            ...
            className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm"
          >
            {DESIGNER_FILTERS.sort.map((option) => (
              <option key={option.value} value={option.value}>
                {option.label}
              </option>
            ))}
      ...
      {showDrawer && selectedDesigner && (
        <DesignerDrawer
          ...
          onApprove={() => handleApprove(selectedDesigner.id)}
          onReject={() => setShowRejectModal(true)}
          onSuspend={() => handleSuspend(selectedDesigner.id)}
          onReactivate={() => handleReactivate(selectedDesigner.id)}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to reject this designer? This action cannot be undone."
        confirmLabel="Reject"
        cancelLabel="Cancel"
        variant="danger"
...
const DesignerDrawer = ({ designer, onClose, onApprove, onReject, onSuspend, onReactivate, isProcessing }: DesignerDrawerProps) => {
    ...
    <>
      ...
      <div className="fixed right-0 top-0 z-50 h-full w-full max-w-2xl overflow-y-auto bg-white shadow-xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Designer Details</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 pb-16 space-y-6">
          <div>
            <h3 className="text-sm font-medium text-gray-700 mb-2">Designer Information</h3>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-3">
              <div className="flex items-center gap-3">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-100">
                  {designer.avatarUrl ? (
                    <img src={designer.avatarUrl} alt="" className="h-full w-full object-cover" />
                  ) : (
                    <Palette className="h-8 w-8 text-gray-500" />
                  )}
                </div>
                <div>
                  <p className="text-lg font-semibold text-gray-900">{designer.name}</p>
                  <StatusBadge status={designer.status} />
                </div>
              </div>
              <div>
                <span className="text-xs text-gray-500">Designer ID</span>
                <p className="text-sm font-medium text-gray-900">{designer.id}</p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Last Activity</span>
                <p className="text-sm font-medium text-gray-900">{designer.lastActivity}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <WalletCopy address={designer.wallet} />
            </div>
          </div>

          <div>
            <h3 className="text-sm font-medium text-gray-700 mb-2">Portfolio Statistics</h3>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500">Submitted Designs</span>
                <span className="text-sm font-medium text-gray-900">{designer.submittedCount}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500">Approved Designs</span>
                <span className="text-sm font-medium text-gray-900">{designer.approvedCount}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500">Approval Rate</span>
                <span className="text-sm font-medium text-gray-900">
                  {designer.submittedCount > 0
                    ? ((designer.approvedCount / designer.submittedCount) * 100).toFixed(1)
                    : '0'}
                  %
                </span>
              </div>
